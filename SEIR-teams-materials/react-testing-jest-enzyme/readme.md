...
We can run `$ cd testing-lesson` then `$ npm test` right now and see what happens.
...
```
Right now, our test fails. When we run `npm test` we get the following error:
...
Bonus: look at the completed application using `npm start` and then style the application accordingly.
