...
public class ShoppingSQLiteOpenHelper extends SQLiteOpenHelper {
    ...
    public static final String[] SHOPPING_COLUMNS = {COL_ID, COL_ITEM_NAME, COL_ITEM_DESCRIPTION,
            COL_ITEM_PRICE, COL_ITEM_TYPE, COL_ITEM_CHECKED};
    ...
    public long addItem(String name, String description, String price, String type) {
        ...
        values.put(COL_ITEM_CHECKED, 0); // default new items to not checked off
        ...
        if (cursor.moveToFirst()) {
            while (!cursor.isAfterLast()) {
                shoppingItems.add(new ShoppingItem(
                        ...
                        (cursor.getInt(cursor.getColumnIndex(COL_ITEM_CHECKED)) == 1)));
                cursor.moveToNext();
            }
        }
    ...
    public ShoppingItem getShoppingItemById(int id) {
        ...
        if (cursor.moveToFirst()) {
            ...
            boolean isChecked = cursor.getInt(cursor.getColumnIndex(COL_ITEM_CHECKED)) == 1;

            cursor.close();
            return new ShoppingItem(id, name, description, price, type, isChecked);
        } else {
            cursor.close();
            return null;
        }
    ...
    public void updateItemCheckedStatus(int itemId, boolean isChecked) {
        ...
        contentValues.put(COL_ITEM_CHECKED, isChecked ? 1 : 0);

        SQLiteDatabase db = getWritableDatabase();

        db.update(SHOPPING_LIST_TABLE_NAME,
                contentValues,
                COL_ID + " = ?",
