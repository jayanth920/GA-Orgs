...
1. <details><summary>Verify mongodb is running</summary>
   ...
   brew services list
   # If the status is not started, run:
...
In the case of our API though, we'll use a pre-existing middleware called [`express.json()`](https://expressjs.com/en/api.html#express.json) instead of writing our own.
...
- Is there anything in the `app/models` directory that's not familar from the
  Mongoose lesson?
...
```bash
node lib/load_books.js bob@website mypassword
```
