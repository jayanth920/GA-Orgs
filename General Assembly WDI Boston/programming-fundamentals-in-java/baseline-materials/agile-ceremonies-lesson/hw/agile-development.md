...
When we practice **TDD** we will be able to write integration (user acceptance) tests that will match our user stories. This is really cool. If done correctly, our tests will automatically check to see that all of our functionalities (pathways) are available and working. Check out the example below showing integration specs written in Ruby using a tool called Capybara. It reads like plain English:
