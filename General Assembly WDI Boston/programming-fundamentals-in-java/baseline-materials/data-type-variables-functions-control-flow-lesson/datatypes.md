...
- Write test cases that identify and describe the Java data types and use cases
...
| 15 min  | [Demo](#demo-lets-start-with-numbers-15-mins)  | Lets start with Numbers |
...
- Byte, which is eight bits of data. These are the same B in MB, GB, TB when talking about space.  You don't need to worry about it right now.  
- Collections (we'll talk more about this later)
...
Create a class called `DataTypeVariable` and `DataTypeVariableTest`. We will use `JUnit` to test each method in this lesson.
...
How about we create a method in our class that takes two numbers and divides them.
First let's write the method signature in `DataTypeVariable`:
...
Then our test in `DataTypeVariableTest`:
```java
@Test
public void testIntDivision(){
    int result = 5/2;
    assertEquals(result, data.intDivision(5,2));
}
```

Finally let's finish our method to make our test pass:
```java
public int intDivision(int dividend, int divisor){
    int quotient = 0;
...
Try changing the `result` variable in your code to divide 5.0f/2.0f. Does your test still pass?
**HINT**: You need to add a third parameter to your `assertEquals()`.
...
We can test this by writing more tests! (from here on out on we will follow the
  pattern of `TDD` for all the demos)
...
public void testFloatingPointDivision(){
    ...
    assertEquals(result, data.floatingPointDivision(5.0f,2.0f), DELTA);
}
```
...
public void testDoubleDivision(){
    double result = 5d/2d;
    assertEquals(result, data.doubleDivision(5d,2d), DELTA);
}
```
...
Let's try add to our calculator class and test each method, the implementation
should look something like this:

```java
System.out.println(cal.adds(2,2));
System.out.println(cal.subtracts(2,2));
System.out.println(cal.divides(2,2)); // this is given
System.out.println(cal.multiplies(2,2));
System.out.println(cal.mods(2,2));
```
