...
Up to this point, we have focused on domains that only have two models, i.e. `Artist`, and `Song`, which in turn have a strict one-to-many relationship. At its core, we expressed these relationships with ActiveRecord methods, and linked the tables via a foreign key on the child table.

This type of relationship is the most common, but today we will be looking at another widely used and very useful relationship that will help build out additional features within our Rails apps.
...
many-to-many relationship, we need a join table.
...
In pairs, spend **10 minutes** answering the following questions for the below pairs of models...  
  ...
  3. What could be a useful additional column to include in the join table (e.g., `order`)?
...
> A `User` model and user authentication functionality has already been provided for you. Because of this, you may see some code in here -- particularly in `models/user.rb` and `routes.rb` that was added by the Devise gem
...
### Testing Our Associations (10 minutes / 1:10)
...
* These will not correspond to the standard resource routes.
...
### You Do: Update Songs Controller (20 minutes / 1:55)
